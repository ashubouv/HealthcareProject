...
interface SessionContextValue {
  ...
  /** End the session and return to the login screen. Data is untouched. */
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      try {
        ...
        setUserId(user.id)
        setMethod((user.method as SignInMethod) ?? null)
        setContact(user.contact ?? null)
        setPersons(persons)
  ...
  const signOut = useCallback(async () => {
    try {
      await api.logout() // invalidate the session server-side
    } catch {
    ...
    // knownUser is kept, so the welcome screen says "Log in".
