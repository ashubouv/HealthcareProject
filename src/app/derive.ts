...
function canonicalName(name: string): string {
  ...
  c = c
    ...
    .replace(/-/g, ' ') // hyphens are spelling noise: "Non-HDL" = "Non HDL", "B-12" = "B 12"
    .replace(/[^a-z0-9%/+ ]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
  ...
  c = c
    .split(' ')
    .map((w) => (w.length >= 5 && w.endsWith('s') && !w.endsWith('ss') && !w.endsWith('us') ? w.slice(0, -1) : w))
    .join(' ')
  ...
  c = c
    ...
    .trim()
  c = TEST_SYNONYMS[c] ?? c
  // Filler words never distinguish tests: "Platelet Count" = "Platelets",
  // "Hemoglobin Estimation" = "Hemoglobin", "MCHC Level" = "MCHC",
  // "Total Cholesterol" = "Cholesterol". (Special "total" names like Total
  // Leukocyte Count / Total Bilirubin were already resolved by the synonym map
  // above, so dropping the word here is safe.)
  const FILLER = new Set([
    ...
    'level',
    'total',
    // repeat-report qualifiers — a "latest"/"previous" value is still the SAME
    // test and must combine into the one measure:
    'latest',
    'previous',
    'current',
    'recent',
  ...
  const kept = c.split(' ').filter((w) => !FILLER.has(w))
  if (kept.length > 0) c = kept.join(' ')
  c = TEST_SYNONYMS[c] ?? c
...
function sameTestFuzzy(a: string, b: string): boolean {
  ...
  // Rule 1b — dual-labelled names: "hba1c glycated hemoglobin" ↔ "hba1c".
  // The longer name contains the shorter one, and its EXTRA words are, by the
  // synonym map, just another name for the same test.
  const dualLabel = (short: string[], long: string[]) => {
    if (short.length >= long.length) return false
    const set = new Set(short)
    if (long.filter((w) => set.has(w)).length !== short.length) return false
    const extra = long.filter((w) => !set.has(w)).join(' ')
    if (!extra) return false
    const shortKey = short.join(' ')
    const mapped = TEST_SYNONYMS[extra]
    return mapped === shortKey || (mapped != null && mapped === TEST_SYNONYMS[shortKey])
  }
  if (dualLabel(aw, bw) || dualLabel(bw, aw)) return true

  if (aw.length !== bw.length) return false

  // Rule 1c — same words, different order: "Bilirubin Direct" ↔ "Direct Bilirubin".
  if (aw.length >= 2 && [...aw].sort().join(' ') === [...bw].sort().join(' ')) return true
  ...
  return (
    [...groups.values()]
      .map(({ display, rows }) => {
        ...
        // Never show repeat-report qualifiers in the measure's name.
        const cleanDisplay =
          display.replace(/\b(latest|previous|current|recent|repeat)\b/gi, '').replace(/\s+/g, ' ').trim() || display
        return { name: cleanDisplay, latest, readings, tone: latest.flag ? 'alert' : 'neutral' } as LabGroup
      })
