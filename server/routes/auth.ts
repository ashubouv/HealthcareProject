...
/** Check an emailed code. On success the code is consumed (single-use). */
async function consumeOtp(
  ...
  const found = await query<{ id: string; code_hash: string; attempts: number; expired: boolean }>(
    `select id, code_hash, attempts, (expires_at < now()) as expired
     from otp_codes
     where method = 'email' and contact = $1
  ...
  if (!codesMatch(row.code_hash, hashCode(String(code)))) {
    ...
    return { ok: false, status: 400, error: 'That code isn’t right. Check the email and try again.' }
  }
  // Success — codes are single-use; clear every outstanding code for this address.
  await query(`delete from otp_codes where method = 'email' and contact = $1`, [email])
  return { ok: true }
}
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const email = String(contact).trim().toLowerCase()
    const check = await consumeOtp(email, code)
    if (!check.ok) {
      res.status(check.status).json({ error: check.error })
      return
    }
    res.json(await startSession('email', email))
  }),
)

// Forgot password: verify the emailed code, set the new password, sign in.
authRouter.post(
  '/reset-password',
  asyncHandler(async (req, res) => {
    ...
    const existing = await query<{ id: string }>(
      `select id from users where method = 'email' and contact = $1`,
      [email],
    )
    ...
    await query(`update users set password_hash = $1 where method = 'email' and contact = $2`, [
      hash,
      email,
    ])
    clearFails(email) // a successful reset lifts any wrong-password lockout
    res.json(await startSession('email', email))
  }),
)
