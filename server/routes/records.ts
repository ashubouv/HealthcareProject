...
// GET /api/records/export — the account's complete structured record as one
// downloadable JSON file: every patient, their notes, and every extracted
// document. A user-held backup, independent of the app's own database.
recordsRouter.get(
  '/export',
  asyncHandler(async (req: AuthedRequest, res) => {
    const persons = await query(
      `select id, full_name as "fullName", age_years as "ageYears", relationship,
              gender, notes, created_at as "createdAt"
       from persons where user_id = $1 order by created_at`,
      [req.userId],
    )
    const records = await query(
      `select ${COLS} from records where user_id = $1 order by created_at`,
      [req.userId],
    )
    res.setHeader('Content-Type', 'application/json')
    res.setHeader('Content-Disposition', 'attachment; filename="healthkeeper-backup.json"')
    res.json({
      app: 'HealthKeeper',
      exportedAt: new Date().toISOString(),
      patients: persons.rows,
