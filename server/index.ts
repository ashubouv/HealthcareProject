...
import { migrate, wipe, query } from './db.js'
...
// Health check. Touches the database so the keep-awake self-ping ALSO counts as
// database activity — free Supabase projects pause after ~a week without any —
// and so this endpoint reports whether the database is reachable.
app.get('/api/health', async (_req, res) => {
  try {
    await query('select 1')
    res.json({ ok: true, db: true })
  } catch {
    res.json({ ok: true, db: false, hint: 'Database unreachable — if Supabase is paused, restore it from the Supabase dashboard.' })
  }
})
...
// Serve IMMEDIATELY — the app must come up even when the database is briefly
// unreachable (free Postgres instances pause/restart). Migrations then retry in
// the background until the database answers; exiting here would put the whole
// service into a crash loop and take the app down entirely.
app.listen(PORT, () => {
...
async function runMigrationsWithRetry(): Promise<void> {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      console.error(`[db] migration attempt ${attempt} failed: ${reason} — retrying in 15s`)
      await new Promise((r) => setTimeout(r, 15_000))
    }
  }
}

void runMigrationsWithRetry().then(() => {
  ...
  // and because /api/health now queries the database, it keeps the free
  // database instance active too.
